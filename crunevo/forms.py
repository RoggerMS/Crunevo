...
class RegisterForm(FlaskForm):
    name = StringField(
        "Nombre Completo", validators=[DataRequired(), Length(min=2, max=100)]
    )
    email = StringField("Correo Electrónico", validators=[DataRequired(), Email()])
    ...
    profile_picture = FileField(
        "Foto de Perfil", validators=[Optional()]
    )  # Validator for file type can be added
